...
class UltimateRecommenderV7_0:
    def __init__(self, popularity, mappings):
        ...
        print("Loading IP History...")
        with open('src/ip_history.pkl', 'rb') as f:
            self.ip_history = pickle.load(f)
            
        from src.domain_rules import BIOLOGICAL_TRANSITIONS
        self.bio_trans = BIOLOGICAL_TRANSITIONS
        
        # 4. Search Matcher (Restored)
        p_df['name'] = p_df['name'].fillna('')
        self.search_matcher = SearchQueryMatcher(p_df)
    ...
    def predict(self, session_features, n=6):
        ...
        ip_addr = session_features.get('ip_address')
        ...
        # --- STAGE 1.5: IP HISTORY (Household/Proxy Fallback) ---
        if len(recommendations) < 3 and ip_addr and ip_addr in self.ip_history:
            for p in self.ip_history[ip_addr]:
                if int(p) not in seen:
                    recommendations.append(int(p))
                    seen.add(int(p))
                if len(recommendations) >= 3: break
        ...
        # --- STAGE 2.5: SEARCH QUERIES ---
        if len(recommendations) < n and search_queries:
            for query in search_queries:
                search_recs = self.search_matcher.match_query(query, n=10)
                for p in search_recs:
                    if int(p) not in seen:
                        recommendations.append(int(p))
                        seen.add(int(p))
                    if len(recommendations) >= n: break
...
if __name__ == "__main__":
    ...
    # Feature extraction includes city AND IP
    v_test_data = v_test[['visit_id', 'counter_user_id_hash', 'region_city', 'ip_address']]
    ...
    vid_to_ip = dict(zip(v_test_data['visit_id'], v_test_data['ip_address']))
    ...
    for visit_id in tqdm(vids, desc="Predicting"):
        ...
        features['ip_address'] = vid_to_ip.get(visit_id)
